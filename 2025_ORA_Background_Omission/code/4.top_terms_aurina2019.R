...
for (bg_type in c('annotation','protein')){
  for (annot2test in c('GO_BP','GO_MF')){
...
p <- ggplot(top_pathways, aes(x = psig.protein, y = Pathway, fill = ratio)) +
  ...
  labs(
    title = sprintf(" k=%d (%d%%)", k, k_perc),
    x = sprintf("Probability of significance")
  )
 plist[[paste(rdonor, k_perc)]] <- p
}

 final_p=wrap_plots(plist, nrow=3, ncol = 1) + 
 plot_annotation(title = sprintf("Top 10 Pathways"),
  theme = theme(plot.title = element_text(size = 14, hjust = 0.5,face = "bold")))

ggsave(sprintf('%s/full_Top10_pathways.png', res_path), plot = final_p, width =4, height =10, dpi = 300)
