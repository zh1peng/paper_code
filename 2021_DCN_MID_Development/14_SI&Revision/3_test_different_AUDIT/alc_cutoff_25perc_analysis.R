...
# a=c(0:100)
# quantile(a)
quantile(fu2_df$audit_total)

fu2_df=fu2_df %>% mutate(fu2_alc=case_when(audit_total>7~1,
                                           audit_total<3~0)) 
fu2_df$fu2_alc=fu2_df$fu2_alc
bsl_df$fu2_alc=fu2_df$fu2_alc
...
df=df %>% filter(complete.cases(fu2_alc))

df$fu2_alc=as.factor(df$fu2_alc)
...
write.csv(result_df,'/Users/zhipeng/Google Drive/post-doc/MID_network_FU2/process_code/intersect_cov/AUDIT analysis new subid/revision_analysis/alc_25percl2h7_results.csv')
...
p1=longitudinal_plot_by_sep(d,'Time','AUDIT Total','')+theme_classic()+theme(legend.position = 'none')
  # geom_hline(aes(yintercept=5.2),color = 'lightgray', alpha = .5,linetype=2) +
  # geom_segment(aes(x=0,xend=0.8,y=2.4,yend=2.4),color = 'lightgray', alpha = .5,linetype=2)+
  # geom_text(aes(2.5,5,label = 'T2 Top 25%', vjust = -1.2))+
  # geom_text(aes(2.5,5,label = 'T2 Bot 25%', vjust = +1.4))+
  # geom_text(aes(0.5,2.4,label = 'T1 cutoff (≥2)', vjust = -0.4))
...
ggsave('/Users/zhipeng/Google Drive/post-doc/MID_network_FU2/process_code/intersect_cov/AUDIT analysis new subid/revision_analysis/alc_25percl3h7_interaction_plot.tiff',
