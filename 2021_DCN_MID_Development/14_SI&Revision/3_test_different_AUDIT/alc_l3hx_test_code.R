...
for (cutoff2test in c(5,6,7,8,9,10,11,12)){
...
bsl_df=df.orig %>% filter(time==1)
fu2_df=df.orig %>% filter(time==2)
fu2_df=fu2_df %>% mutate(fu2_alc=case_when(audit_total>cutoff2test~1,
                                           audit_total<cutoff2test+1~0))
...
result_df$audit_cutoff=cutoff2test

all_result=rbind(all_result,result_df)
}

all_result$audit_cutoff=all_result$audit_cutoff+1
write.csv(all_result,'/Users/zhipeng/Google Drive/post-doc/MID_network_FU2/process_code/intersect_cov/AUDIT analysis new subid/revision_analysis/alc_l3hx_results.csv')



# make plot 
plist=list()
title2show=c('Shortest path length','Strength','Clutering coefficient','Left middle temporal gyrus')
measures=c('wu_net_aLp','wu_net_adeg','wu_net_aCp','mgt_vsr_node_MTG')
for (m_i in c(1:4)){

df2plot=all_result %>% filter(grepl(sprintf('%s*',measures[m_i]),rownames(.)))


p=ggplot(df2plot,aes(x=audit_cutoff,y=int_t))+
  geom_point(shape=18,color='darkorange',size=2)+
  geom_errorbar(aes(ymin=int_l025,ymax=int_u975),color='dodgerblue')+
  ylim(-4,5)+
  ylab('Interaction t values')+
  xlab('T2 AUDIT cutoff')+
  ggtitle(sprintf('%s',title2show[m_i]))+
  theme_classic()
plist[[m_i]]=p
}
main=grid.arrange(grobs=plist, nrow = 2)
ggsave('/Users/zhipeng/Google Drive/post-doc/MID_network_FU2/process_code/intersect_cov/AUDIT analysis new subid/revision_analysis/l3hx_interaction_plot.tiff',
       main,width = 6,height = 6)
...
d=data.frame(x=as.numeric(all_df$time),
             y=all_df$audit_total,
             id=all_df$subcode,
             sep=all_df$fu2_alc)
...
p1=longitudinal_plot_by_sep(d,'Time','AUDIT Total','')+theme_classic()+theme(legend.position = 'none')+
  ...
  geom_segment(aes(x=0,xend=0.8,y=2.4,yend=2.4),color = 'lightgray', alpha = .5,linetype=2)+
  geom_text(aes(0.5,7,label = 'T2 cutoff (≥8)', vjust = -1.2))+
  geom_text(aes(0.5,2.4,label = 'T1 cutoff (≥3)', vjust = -0.8))
...
ggsave('/Users/zhipeng/Google Drive/post-doc/MID_network_FU2/process_code/intersect_cov/AUDIT analysis new subid/revision_analysis/l3h8_interaction_plot.tiff',
