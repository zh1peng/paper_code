...
# 2020/08/13
# 2020/10/15 updated
# 2021/09/17 add effect size output
# zhipeng
...
# the interaction of var1 and var2 will be extracted as well
 ...
 nonnan_sum=sub_n-nan_sum
  if (nan_sum<(0.5*sub_n) & zero_sum<(0.5*nonnan_sum)){ #if <half of voxels are neither nan nor zero, run the following:
    ...
    tmp_data.complete=tmp_data[which(complete.cases(tmp_data)),]
    ...
    es1_val=d(summary(tmp_lme_model)$tTable[var2extract1,"DF"],
      summary(tmp_lme_model)$tTable[var2extract1,"t-value"],
      as.numeric(table(tmp_data.complete$time)["1"]),
      as.numeric(table(tmp_data.complete$time)["0"]))

    es2_val=d(summary(tmp_lme_model)$tTable[var2extract2,"DF"],
      summary(tmp_lme_model)$tTable[var2extract2,"t-value"],
      as.numeric(table(tmp_data.complete$time)["1"]),
      as.numeric(table(tmp_data.complete$time)["0"]))
    ...
    es1_val=NaN
    es2_val=NaN}
  return(cbind(t1_val,p1_val,t2_val,p2_val,es1_val,es2_val))
}
...
# 
# Output:
# result_list: beta/t/p values corresponding to var2extract1 var2extract2 and their interaction
...
es1_vals=unlist(lapply(tmp_results, function(i) as.numeric(i[,'es1_val'])))
es2_vals=unlist(lapply(tmp_results, function(i) as.numeric(i[,'es2_val'])))
...
# note: numeric data are centered here
...
es1_write=array(result_list$es1_vals,dim_info)
t2_write=array(result_list$t2_vals,dim_info)
p2_write=array(result_list$p2_vals,dim_info)
es2_write=array(result_list$es2_vals,dim_info)
...
f.write.nifti(es1_write,sprintf('es1_vals_%s.nii',filename_info),nii=TRUE)
...
f.write.nifti(es2_write,sprintf('es2_vals_%s.nii',filename_info),nii=TRUE)
}
...
shuffle_time_alc <- function(cov){
  ...
  # for the subjects that are shown in both BSL and FU2
  # shuffle within subjects to prevent same subject in same wave
  cov$shuffled_time=ave(cov$time, cov$subcode,FUN=sample) #time is shuffled within subjects
  return(cov)
}
...
#======================================shuffle time and alc label in cov==================================================
shuffle_time_sex <- function(cov){
  bsl_df=cov%>% filter(time==1)
   tmp_sex=sample(bsl_df$sex) # sex is same for bsl and fu2, so it is shuffled between subjects
     cov$shuffled_sex=fct_c(tmp_sex,tmp_sex)
  # for the subjects that are shown in both BSL and FU2
  # shuffle within subjects to prevent same subject in same wave
  cov$shuffled_time=as.factor(ave(cov$time, cov$subcode,FUN=sample)) #time is shuffled within subjects
  return(cov)
}
