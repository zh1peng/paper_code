---
title: "S10: Analysis using the partial least square (PLS) regression coefficients"
output:
...
|----------|--------------------------|--------|---------------------------------------|-------------------------|-----------------------|------------------------|-----------------------------------------|
| S10     | Desikan (34)             | 0.4    | 1000 simulated maps (Moran's I=0.03) | 500 simulated gene sets | PLS1 Regression Coefficients   | Competitive / Self-contained | Mean, Meanabs, Meansqr, Maxmean, Median, KS, Weighted KS |

*Note: The significant number of associations was not feasible as an aggregation method for this analysis.*
...
### 4.2. Examine the correlation between Kurtosis and Psig-B

```{r message=FALSE, warning=FALSE, paged.print=FALSE}
# For PLS correlation, Kurtosis of the PLS1 regression coefficients better explained the variance of Psig-B than bimodality.
brain_info=get_brain_info(data_path=sprintf('%s/data',project_path),
                          atlas=atlas,
                          rdonor=rdonor,
                          brain_type=brain_type,
                          method=cor_type)
brain_res.nested.list=lapply(psig.list, correlate_psig_with_info,info=brain_info,var2test='kurtosis')
...
brain_res.plot.list=lapply(brain_res.nested.list,plot_res.nested,
                           annot_position=c(1.5,0.95),
                           xlim2show=c(1,5),
                           ylim2show=c(-0.05,1))
```

### 4.2.1. Plot correlation between Psig-B and kurtosis
#### Figure 5. Results of the kurtosis analysis for the competitive (A) and self-contained null model (B). The x-axis indicates the kurtosis of the PLS1 regression coefficients between a specific brain map and transcriptional profiles of background genes. The y-axis indicates the probability of significance for a specific brain map (Psig-B). Each dot denotes a brain map and the horizontal dashed line denotes a Psig-B value of 0.05.
```{r include=F}
p3=grid.arrange(grobs=brain_res.plot.list[[null_type_level[1]]],
                ncol=2,
                top = textGrob(sprintf("A. %s",null_type_label[1]),gp=gpar(fontsize=16,font=1),x = -0.01, hjust = 0),
                left =textGrob("Psig-B",gp=gpar(fontsize=12,font=2),rot=90),
                bottom=textGrob("Kurtosis",gp=gpar(fontsize=12,font=2)))
...
### 4.2.2. Report correlation between Psig-B and Kurtosis
...
### 4.3. Examine the correlation between BI-dip and Psig-B
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
...
### 4.3.2. Report correlation between Psig-B and BI-dip
```{r,eval=FALSE,echo=TRUE}
df1=brain_res.report.list[[null_type_level[1]]]
df2=brain_res.report.list[[null_type_level[2]]]
kable(df1,caption = sprintf("A. %s",null_type_label[1]))
