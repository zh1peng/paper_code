...
covs = c("Age", "Sex", "Site","Alclast30days","DependentonPrimaryDrug")
sig_regions=c('AI_accumb','AI_postcentral_surfavg','AI_fusiform_surfavg')
...
for (region in sig_regions)
{
  ...
  lme_models[[region]] <- lme(outcome ~ Alclast30days + Sex + Age, random=~1|Site, data = aidata.complete, method="ML", na.action="na.fail")
  lme_null_models[[region]] <- lme(outcome ~ Sex + Age, random=~1|Site, data = aidata.complete, method="ML", na.action="na.fail")
  ## calculate cohen's d effect size
  d_dx[[region]] <- d(summary(lme_models[[region]])$tTable["Alclast30days","t-value"], N.cases[[region]])
...
table[,"b_Alclast30days"] <- as.numeric(sapply(data.models, function(x){x[,"Value"]["Alclast30days"]}))
table[,"se_Alclast30days"] <- as.numeric(sapply(data.models, function(x){x[,"Std.Error"]["Alclast30days"]}))
table[,"t_Alclast30days"] <- as.numeric(sapply(data.models, function(x){x[,"t-value"]["Alclast30days"]}))
table[,"p_Alclast30days"] <- as.numeric(sapply(data.models, function(x){x[,"p-value"]["Alclast30days"]}))
