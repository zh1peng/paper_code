...
regions_list=list(#L_ct=colnames(all_site_es)[grep('^L_.*\\_thickavg$',colnames(all_site_es))],
                 ...
                 L_sa=colnames(all_site_es)[grep('^L_.*\\_surfavg$',colnames(all_site_es))],
                 R_sa=colnames(all_site_es)[grep('^R_.*\\_surfavg$',colnames(all_site_es))],
                 L_sv=c("L_thal" ,"L_caud","L_put", "L_pal","L_hippo","L_amyg", "L_accumb"),
                 R_sv=c("R_thal" ,"R_caud","R_put", "R_pal","R_hippo","R_amyg", "R_accumb"))
...
mu ~ dnorm(0,10)
itau   ~ dgamma(1.1051,0.1051) # mode 1.0, sd 10.0
tau <- pow(1/itau,1/2)
icommon_sigma ~ dgamma(1.1051,0.1051)  # mode 1.0, sd 10.0
